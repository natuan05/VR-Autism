...
- [ ] **Dọn dẹp:** Xóa toàn bộ dead code (`TimeManager`, `FirebaseManager`, v.v.).
- [ ] **Chuẩn hóa kiến trúc:** Sửa lại 3 loại Singleton về một chuẩn duy nhất, an toàn (Duplicate guard).
- [ ] **Quy hoạch Namespace:** Áp dụng `VRAutism.Core`, `VRAutism.Cloud`, `VRAutism.Player` cho toàn project.
- [ ] **Bảo mật & Clean Code:** Đưa các đường dẫn Firebase ra file Const (`FirebasePaths.cs`), đóng gói property (Encapsulation) trong `QuestController`.

#### 1.2 Tái cấu trúc Model Dữ liệu (Local)
- [ ] Tách biệt hoàn toàn các lớp Data Models (`LessonTimeData`, `QuestTimeData`, `SkillsData`) sang thư mục riêng.
